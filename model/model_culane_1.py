...
class parsingNet(torch.nn.Module):
    def __init__(self, pretrained=True, backbone='50', num_grid_row = None, num_cls_row = None, num_grid_col = None, num_cls_col = None, num_lane_on_row = None, num_lane_on_col = None, use_aux=False,input_height = None, input_width = None, fc_norm = False):
        ...
        if backbone in ['34','18', '34fca']:
            self.pool = torch.nn.Conv2d(512,256,1)     
        else:
            raise ValueError
        ...
        #     )
        self.cls_loc = torch.nn.Sequential(
            torch.nn.Conv2d(32,8,3,1),
            torch.nn.Flatten(),
            torch.nn.LayerNorm(1536*2) if fc_norm else torch.nn.Identity(),
            torch.nn.ReLU(),
            torch.nn.Linear(1536*2,self.dim1+self.dim2)# 57600)
        )
        ...
        # )
        self.cls_ext_row = torch.nn.Sequential(
            torch.nn.Conv2d(32,4,3,2),
            torch.nn.Flatten(),
            torch.nn.LayerNorm(192*2) if fc_norm else torch.nn.Identity(),
            torch.nn.ReLU(),
            torch.nn.Linear(192*2,self.dim3+self.dim4)#576)
        )
        self.cls_lan_cls = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Linear(384,self.cls_out))
